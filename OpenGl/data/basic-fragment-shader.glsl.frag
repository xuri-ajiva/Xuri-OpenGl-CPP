...
void main()
{
    ...
    vec3 diffuse  = u_directional_light.diffuse  * u_material.diffuse *      max( dot(normal, light),    0.0);
    ...
    diffuse   += attentuation * u_point_light.diffuse  * u_material.diffuse  *      max( dot(normal, light),    0.0);
    ...
    if(theta > u_spot_light.outerCone) {
        ...
        diffuse  += u_spot_light.diffuse  * intensity * u_material.diffuse  *      max( dot(normal, light),    0.0);
